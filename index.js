...
const currlist = wordlist.slice(0, NUM_WORDS * NUM_PLAYERS)
wordlist = wordlist.slice(NUM_WORDS * NUM_PLAYERS)
const scores = new Array(NUM_PLAYERS).fill(0)
const names = new Array(NUM_PLAYERS).fill('')
...
io.on('connection', function (socket) {
  ...
  socket.on('DEL word', function (id) {
    ...
    const num = parseInt(id.substr(4))
    const newword = wordlist.shift()
    currlist[num] = newword
    console.log('NEW word: ' + newword + ' ' + id)
    io.emit('NEW word', newword, id)

    // Increase score
    const playerNum = Math.floor(num / NUM_WORDS)
    scores[playerNum]++
